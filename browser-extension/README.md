...
- 默认目标缓冲 30 秒：非片尾缓冲连续 10 秒不增长，或前向缓冲从高位降至 10 秒时切换。
- 播放位置不动时每 4 秒继续尝试下一个候选。
- 换源时只中断当前媒体请求，后续 Range 请求改走新 CDN；不会重建 `<video>` 或 `MediaSource`，已有 MSE 缓冲会保留。
- 已失败主机在当前视频内不再使用，并进入默认 30 秒冷却。
- AV1 仅在浏览器通过 `MediaSource.isTypeSupported` 报告支持时优先。
...
点击工具栏图标可查看当前 CDN、缓冲和切换原因，也可手动换源。完整阈值在扩展设置页中调整。
